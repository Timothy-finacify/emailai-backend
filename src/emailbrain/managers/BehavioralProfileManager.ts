...
export class BehavioralProfileManager {
  ...
  static async processCampaignFeedback(
    ...
    updated.learned_triggers = this.updateLearnedTriggers(
      updated.learned_triggers,
      feedback
    );

    // Step 2: Update subject line patterns
    updated.learned_subject_patterns = this.updateSubjectPatterns(
      updated.learned_subject_patterns,
      feedback
    );

    // Step 3: Update send time performance
    updated.learned_send_times = this.updateSendTimes(
      updated.learned_send_times,
      feedback
    );

    // Step 4: Update CTA performance
    updated.learned_cta_performance = this.updateCTAPerformance(
      updated.learned_cta_performance,
      feedback
    );

    // Step 5: Track engagement decay
    updated.engagement_decay_curves = this.updateEngagementDecay(
      updated.engagement_decay_curves,
      feedback
    );

    // Step 6: Update persona micro-adjustments
    updated.persona_overrides = this.updatePersonaOverrides(
      updated.persona_overrides,
      feedback
    );

    // Step 7: Track real objections
    if (feedback.objections_mentioned) {
      updated.real_objections = this.trackObjections(
        updated.real_objections,
        feedback
      );
    }
    ...
    updated.data_quality = this.updateDataQuality(
  ...
  private static updateLearnedTriggers(
    triggers: LearnedTrigger[],
    feedback: CampaignFeedback
  ): LearnedTrigger[] {
    const triggerUsed = feedback.primary_trigger_used;

    // Calculate success metrics
    const emailWasSuccess =
      feedback.actual_open_rate > 0.2 && feedback.actual_click_rate > 0.05;
    ...
    const engagementLift =
      feedback.actual_open_rate > avgOpenRate ? feedback.actual_open_rate - avgOpenRate : 0;
  ...
  private static updateSendTimes(
    ...
    if (!timeData.by_persona[feedback.persona]) {
      ...
      const personaN =
        (Object.keys(timeData.by_persona).length + 1) || 1;
  ...
  private static updateCTAPerformance(
    ctaPerfs: CTAPerformance[],
    feedback: CampaignFeedback
  ): CTAPerformance[] {
    const ctaUsed = feedback.cta_used;

    let cta = ctaPerfs.find(
      c =>
        c.cta === ctaUsed &&
    ...
    if (!cta) {
      cta = {
        ...
        sampleSize: 0,
        sentiment: feedback.cta_type as any,
   ...
   */
  private static updateEngagementDecay(decayData: any[], feedback: CampaignFeedback): any[] {
  ...
  private static updatePersonaOverrides(
    overrides: Record<string, PersonaMicroAdjustment>,
    feedback: CampaignFeedback
  ): Record<string, PersonaMicroAdjustment> {
    if (!overrides[feedback.persona]) {
      ...
      if (feedback.actual_open_rate > 0.25) {
        ...
        override.trigger_override_confidence = Math.min(
          override.trigger_override_confidence + 0.1,
          1
        );
      }
      ...
      if (feedback.actual_conversion_rate > 0.05) {
        ...
        override.cta_override_confidence = Math.min(
          override.cta_override_confidence + 0.1,
          1
        );
      }
   ...
   */
  private static trackObjections(objections: any[], feedback: CampaignFeedback): any[] {
    if (!feedback.objections_mentioned) return objections;

    feedback.objections_mentioned.forEach(objectionText => {
      let objection = objections.find(
        o => o.objection_statement === objectionText
      );
   ...
   * Analyze trends
   */
  private static analyzeTrends(profile: BehavioralProfile): any {
    return {
   ...
   */
  private static updateDataQuality(quality: any, feedback: CampaignFeedback): any {
   ...
   */
  private static extractSubjectPattern(subject: string): string {
   ...
   */
  static getBestPerformer(
    items: Array<{ conversionRate?: number; successRate?: number; clickRate?: number }>,
    metric: "conversionRate" | "successRate" | "clickRate" = "successRate"
  ): any {
