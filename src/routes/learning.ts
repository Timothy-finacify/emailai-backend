...
import { BehavioralProfileModel } from '../models/BehavioralProfile';
...
router.post('/api/v1/learning/feedback', 
  authenticateUser,  // Authentication required
  ...
  async (req, res) => {
    ...
    try {
      ...
      const existingRecord = await withRetry(async () => {
        return await CampaignRecord.findOne({ emailId }).session(session);
      });
      ...
      const profile = await withRetry(async () => {
        return await BehavioralProfileModel.findOne({ 
      ...
      const updatedProfile = await withRetry(async () => {
        return await BehavioralProfileManager.processCampaignFeedback(feedback, profile);
      });
      ...
      const [profileUpdate, campaignUpdate] = await Promise.all([
        BehavioralProfileModel.updateOne(
          { niche_id: existingRecord.nicheId },
          { 
            ...updatedProfile,
...
router.get('/api/v1/learning/health', authenticateUser, async (req, res) => {
  res.json({ 
    status: 'healthy',
    timestamp: new Date().toISOString(),
    version: '1.0.0'
