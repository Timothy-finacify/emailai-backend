...
exports.getProfile = async (req, res) => {
    try {
        ...
        const user = await User.findById(req.user.id || req.user._id).select('-password -otp -otpExpires -passwordResetToken -passwordResetExpires -loginAttempts -lockUntil');
        if (!user) {
...
exports.updateProfile = async (req, res) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(req.user.id || req.user._id, { name, phone, company, jobTitle, industry, companySize, avatar }, { new: true, runValidators: true }).select('-password -otp -otpExpires -passwordResetToken -passwordResetExpires');
...
exports.changePassword = async (req, res) => {
    try {
        ...
        await sendEmail(user.email, 'Password Changed', 'Your password has been changed successfully. If this was not you, please contact support.');
        res.status(200).json({
            success: true,
...
exports.deleteAccount = async (req, res) => {
    try {
        ...
        await sendEmail(userEmail, 'Account Deleted', 'Your account has been permanently deleted.');
        res.status(200).json({
            success: true,
...
exports.requestEmailVerification = async (req, res) => {
    try {
        ...
        await user.save();
        await sendEmail(user.email, 'Email Verification Code', `Your verification code is: <strong>${otp}</strong><br><br>This code will expire in 10 minutes.`);
