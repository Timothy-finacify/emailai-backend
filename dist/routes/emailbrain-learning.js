 ...
 */
router.post('/api/campaigns/feedback', auth_1.authenticateUser, async (req, res) => {
    try {
        ...
        const existingCampaign = await CampaignRecord_1.CampaignRecordModel.findOne({
        ...
        const currentProfile = await BehavioralProfile_1.BehavioralProfileModel.findOne({
            niche_id: feedback.nicheId
        });
        // Step 2: Process feedback and update profile
        const updatedProfile = await BehavioralProfileManager_1.BehavioralProfileManager.processCampaignFeedback(feedback, currentProfile);
        // Step 3: Save updated profile to MongoDB with atomic operation
        await BehavioralProfile_1.BehavioralProfileModel.updateOne({ niche_id: feedback.nicheId }, { $set: updatedProfile }, { upsert: true });
        // ✅ FIXED: Update campaign status to processed
        await CampaignRecord_1.CampaignRecordModel.updateOne({ email_id: feedback.emailId }, { $set: { status: 'processed', processed_at: new Date() } });
        ...
        res.json({
            ...
            insights: {
                ...
                best_performers: {
                    trigger: BehavioralProfileManager_1.BehavioralProfileManager.getConfidenceWeightedBest(updatedProfile.learned_triggers || []),
                    cta: BehavioralProfileManager_1.BehavioralProfileManager.getBestPerformer(updatedProfile.learned_cta_performance || [], 'conversion_rate')
                }
            }
        });
    }
 ...
 */
router.get('/api/niches/:nicheId/behavioral-profile', auth_1.authenticateUser, async (req, res) => {
    try {
        const { nicheId } = req.params;
        const dbProfile = await BehavioralProfile_1.BehavioralProfileModel.findOne({
            niche_id: nicheId
        });
        if (!dbProfile) {
        ...
        const profile = (0, BehavioralProfile_1.dbToCamelCase)(dbProfile);
        res.json({
            success: true,
            profile,
            recommendations: {
                best_trigger: BehavioralProfileManager_1.BehavioralProfileManager.getConfidenceWeightedBest(dbProfile.learned_triggers || []),
                best_cta: BehavioralProfileManager_1.BehavioralProfileManager.getBestPerformer(dbProfile.learned_cta_performance || [], 'conversion_rate'),
 ...
 */
router.get('/api/niches/:nicheId/trends', auth_1.authenticateUser, async (req, res) => {
    try {
        const { nicheId } = req.params;
        const dbProfile = await BehavioralProfile_1.BehavioralProfileModel.findOne({
            niche_id: nicheId
 ...
 */
router.get('/api/niches/:nicheId/persona/:persona/overrides', auth_1.authenticateUser, async (req, res) => {
    try {
 ...
 */
router.get('/api/niches/:nicheId/objections', auth_1.authenticateUser, async (req, res) => {
    try {
        const { nicheId } = req.params;
        const dbProfile = await BehavioralProfile_1.BehavioralProfileModel.findOne({
            niche_id: nicheId
        ...
        const sortedObjections = (dbProfile.real_objections || []).sort((a, b) => (b.frequency || 0) - (a.frequency || 0));
        res.json({
            success: true,
            niche: nicheId,
 ...
 */
router.post('/api/emails/generate', auth_1.authenticateUser, async (req, res) => {
    try {
        ...
        const dbBehavioralProfile = await BehavioralProfile_1.BehavioralProfileModel.findOne({
            niche_id: nicheId
        });
        ...
        if (dbBehavioralProfile && (dbBehavioralProfile.data_quality?.total_campaigns_tracked || 0) > 5) {
            const bestTrigger = BehavioralProfileManager_1.BehavioralProfileManager.getConfidenceWeightedBest(dbBehavioralProfile.learned_triggers || []);
            ...
            const bestCTA = BehavioralProfileManager_1.BehavioralProfileManager.getBestPerformer((dbBehavioralProfile.learned_cta_performance || []).filter(c => c.persona === persona), 'conversion_rate');
        ...
        await CampaignRecord_1.CampaignRecordModel.create({
            email_id: emailId,
        ...
        res.json({
            ...
            learning_applied: {
                ...
                best_trigger_from_learning: BehavioralProfileManager_1.BehavioralProfileManager.getConfidenceWeightedBest(dbBehavioralProfile?.learned_triggers || [])?.trigger
            }
        });
    }
    catch (error) {
 ...
 * ========== HELPER FUNCTIONS ==========
 */
function estimateOpenRate(subject, persona) {
...
function estimateClickRate(cta, stage) {
    const stageBase = {
        awareness: 0.02,
        consideration: 0.05,
        decision: 0.08,
...
function estimateConversionRate(stage, persona) {
    const stageBase = {
        awareness: 0.005,
        consideration: 0.015,
        decision: 0.05,
...
async function generateEmailWithOpenRouter(context, subject, persona) {
...
function buildEmailPrompt(context, subject, persona) {
...
function generateFallbackEmail(context, persona) {
    const subjectLines = [
...
function extractEmailComponents(text) {
...
//# sourceMappingURL=emailbrain-learning.js.map